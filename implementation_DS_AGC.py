    ...
    for epoch in range(parameter["epochs"]):
        ...
        for data_target, label_target in target_bar:
            ...
            if epoch >= threshold:
                ...
                pred, domain_loss, ajloss, contrastive_loss, sim_weight, L2 = model(
                    torch.cat((data_source, data_target)), tripleada=0, threshold=0
                )
            ...
            target_bar.set_description(
                "sub:{} Train Epoch: [{}/{}] Loss: {:.4f} source_acc:{:.2f}% target_acc:{:.2f}%".format(
                    subject_id,
                    epoch + 1,
                    parameter["epochs"],
                    epoch_train_loss,
        ...
        if best_test_acc < target_test_acc:
            ...
            torch.save(
                model.state_dict(),
                "model_semi_session_{}_batch48_{}epoch_{}U_sub{}.pkl".format(
                    Dataset_name,
                    parameter["threshold"],
                    parameter["num_of_U"],
                    subject_id,
...
def get_dataset_IV(test_id, parameter):
    path = []
    path.append(
        sys_path + "\\de_feature\\feature_for_net_session" + str(1) + "_LDS_de_IV"
    )
    path.append(
        sys_path + "\\de_feature\\feature_for_net_session" + str(2) + "_LDS_de_IV"
    )
    path.append(
        sys_path + "\\de_feature\\feature_for_net_session" + str(3) + "_LDS_de_IV"
    )
...
def cross_subject(
    ...
    torch_dataset_test = Data.TensorDataset(
        torch.from_numpy(target_set["feature"]), torch.from_numpy(target_set["label"])
    )
    torch_dataset_source_labeled = Data.TensorDataset(
        torch.from_numpy(source_set_labeled["feature"]),
        torch.from_numpy(source_set_labeled["label"]),
    )
    torch_dataset_source_unlabeled = Data.TensorDataset(
        torch.from_numpy(source_set_unlabeled["feature"]),
        torch.from_numpy(source_set_unlabeled["label"]),
    )

    source_labeled_loaders = torch.utils.data.DataLoader(
    ...
    source_unlabeled_loaders = torch.utils.data.DataLoader(
...
def main(parameter, net_params):
    ...
    for subject_id in range(sub_num):
        if net_params["category_number"] == 3:
            target_set, source_set_labeled, source_set_unlabeled = get_dataset(
                subject_id, session_id, parameter
            )
            Dataset_name = "SEED"
        elif net_params["category_number"] == 4:
            target_set, source_set_labeled, source_set_unlabeled = get_dataset_IV(
                subject_id, parameter
            )
            Dataset_name = "SEEDIV"
        elif net_params["category_number"] == 5:
            target_set, source_set_labeled, source_set_unlabeled = get_dataset_V(
                subject_id, parameter
            )
            Dataset_name = "SEEDV"
        else:
    ...
    result_list = {"best_acc_mat": best_acc_mat, "target_acc_curve": target_acc_curve}
    os.chdir(sys_path + "\\model_result")
    np.save(
        "result_list_semi_session_{}_batch48_{}epoch_{}U.npy".format(
            Dataset_name, parameter["threshold"], parameter["num_of_U"]
        ),
        result_list,
    )
    return csub
