...
def diff_loss(diff, S, Falpha):
    ...
    if len(S.shape) == 4:
        # batch input
        return Falpha * torch.mean(
            torch.sum(torch.sum(diff**2, axis=3) * S, axis=(1, 2))
        )
    else:
...
class SemiGCL(nn.Module):
    def __init__(self, net_params):
        ...
        self.domain_classifier2_3 = adversarial.DomainAdversarialLoss_threeada(
            hidden_1=64
        )
        self.fea_extrator_f = feature_extractor(310, 64, 64)
        self.fea_extrator_g = feature_extractor(
    ...
    def sim_matrix2(self, ori_vector, arg_vector, temp=1.0):
        for i in range(len(ori_vector)):
            sim = torch.cosine_similarity(
                ori_vector[i].unsqueeze(0), arg_vector, dim=1
            ) * (1 / temp)
            if i == 0:
